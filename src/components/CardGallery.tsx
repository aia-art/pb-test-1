...
import { tryOnNightCafe } from '../utils/forms';
...
function MiniCard({ card, onClick }: { card: Card; onClick: () => void }) {
  ...
  function onMove(e: React.MouseEvent) {
    ...
    ref.current.style.transform = `perspective(600px) rotateY(${x * 20}deg) rotateX(${-y * 20}deg) scale(1.05)`;
    const sh = ref.current.querySelector('.card-shine') as HTMLElement;
  ...
  const cost = card.type === 'creator' ? null
    : card.type === 'model' ? `P${card.playCost ?? 0} / A${card.activateCost ?? 0}`
    : card.cost !== undefined ? `${card.cost}${card.costType === 'reputation' ? ' Rep' : ' Cr'}` : null;
  ...
  return (
    <div ref={ref} onClick={onClick} onMouseMove={onMove} onMouseLeave={onLeave}
      ...
      className="relative bg-[#1c2120]/60 backdrop-blur-sm border border-[#a1d0c6]/10 hover:border-[#a1d0c6]/30 rounded-2xl overflow-hidden cursor-pointer group shadow-lg hover:shadow-[0_0_30px_rgba(161,208,198,0.12)] flex flex-col"
    >
      <div className="card-shine absolute inset-0 rounded-2xl pointer-events-none z-10 transition-all" />
      <div className={`absolute top-0 left-0 right-0 h-1 ${TYPE_COLOR[card.type].split(' ')[0].replace('border-','bg-').replace('/60','').replace('/40','')}`} />

      {/* Art */}
      <div className={`relative w-full aspect-square bg-gradient-to-br ${cardGrad(card)} flex items-end p-2`}>
        <span className="text-[10px] italic text-[#c0c8c5]/40 leading-tight line-clamp-3">{card.illustration}</span>
        <div className="absolute top-2 right-2 flex items-center gap-1">
          {'•'.repeat(card.rarityDots).split('').map((_, i) => (
            <span key={i} className={`w-1.5 h-1.5 rounded-full inline-block ${RARITY_COLOR[card.rarity].replace('text-', 'bg-')}`} />
          ))}
        </div>
      </div>
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-grow">
        ...
        <div className={`text-[10px] px-2 py-0.5 rounded-full border w-fit font-semibold uppercase tracking-wider ${TYPE_COLOR[card.type]}`}>
          {card.type}
        </div>
      </div>
    </div>
...
// ── Ability block (modal) ─────────────────────────────────────
function AbilityBlock({ ab, isPassive = false }: { ab: any; isPassive?: boolean }) {
  const isSig = ab.num === 'signature';
  const costStr = isPassive ? (ab._label || 'Passive')
    : isSig
      ? [ab.cost?.loyalty && `${ab.cost.loyalty} Loyalty`, ab.cost?.reputation && `${ab.cost.reputation} Rep`].filter(Boolean).join(' · ')
  ...
  return (
    <div className={`rounded-xl p-3 border ${isSig ? 'border-[#a1d0c6]/40 bg-[#a1d0c6]/5' : 'border-[#dfe3e1]/5 bg-[#0d1211]/40'}`}>
      ...
      <p className="text-xs text-[#c0c8c5] leading-relaxed"
        dangerouslySetInnerHTML={{ __html: linkTerms(ab.text) }} />
    </div>
  );
}
...
function CardModal({ card, onClose }: { card: Card; onClose: () => void }) {
  ...
  async function handleTryPrompt() {
  ...
  const abilityList = [
    card.passive   ? { ...card.passive,   _label: 'Passive',   num: '◈', name: card.passive.name   } : null,
    card.influence ? { ...card.influence, _label: 'Influence', num: '◈', name: card.influence.name } : null,
    ...(card.abilities || []),
  ].filter(Boolean);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
      ...
      <div className="bg-[#1c2120] border border-[#a1d0c6]/15 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-fade-in max-h-[90vh] flex flex-col">
        ...
        <div className="flex overflow-y-auto flex-1">
          {/* Left: art */}
          <div className={`w-48 shrink-0 bg-gradient-to-br ${cardGrad(card)} flex flex-col justify-between p-3`}>
            <div className="space-y-1">
              {'•'.repeat(card.rarityDots).split('').map((_, i) => (
                <span key={i} className={`w-2 h-2 rounded-full inline-block mr-0.5 ${RARITY_COLOR[card.rarity].replace('text-','bg-')}`} />
              ))}
            </div>
            <p className="text-[10px] italic text-[#c0c8c5]/50 leading-tight">{card.illustration}</p>
            <p className="text-[9px] font-mono text-[#c0c8c5]/25 mt-2">SET 001 · Prompt Battle</p>
          </div>
          ...
          <div className="flex-1 p-5 space-y-4 overflow-y-auto">
            ...
            {(card.compatible || card.incompatible) && (
            ...
            {abilityList.length > 0 && (
              <div className="space-y-2">
                ...
                {abilityList.map((ab, i) => <AbilityBlock key={i} ab={ab} isPassive={!!(ab as any)._label} />)}
              </div>
            )}
            ...
            {card.favouritePrompt && (
              <div className="space-y-2">
                <SectionTitle>Favourite Prompt</SectionTitle>
                <div className="rounded-xl p-3 border border-[#dfe3e1]/5 bg-[#0d1211]/40">
                  <div className="flex items-start gap-2 mb-1.5">
                    ...
                    <span className="text-xs text-[#dfe3e1] font-mono leading-relaxed flex-1">{card.favouritePrompt.text}</span>
                  </div>
                  <p className="text-xs text-[#c0c8c5] leading-relaxed pl-7 mb-2">{card.favouritePrompt.effect}</p>
                  <button onClick={handleTryPrompt}
            ...
            {card.effect && (
              <div className="space-y-2">
                ...
                {card.effect.split('\n\n').map((block, i) => (
                  <p key={i} className="text-xs text-[#c0c8c5] leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: linkTerms(block) }} />
                ))}
              </div>
            )}
            ...
            {card.attachesTo && (
              <div className="space-y-1">
                <SectionTitle>Attaches To</SectionTitle>
                <p className="text-xs text-[#c0c8c5]">{card.attachesTo}</p>
              </div>
            )}
            {card.compatibleModels && (
              <div className="space-y-1">
                <SectionTitle>Compatible Models</SectionTitle>
                <p className="text-xs text-[#c0c8c5]">{card.compatibleModels}</p>
              </div>
            )}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[#0d1211]/60 border border-[#dfe3e1]/5 rounded-lg px-3 py-1.5 flex flex-col items-center min-w-[50px]">
...
export default function CardGallery() {
  ...
  const [activeView,   setActiveView]   = useState<'gallery' | 'bydeck' | 'votes'>('gallery');
  ...
  const filtered = ALL_CARDS.filter(c => {
    ...
    if (search) {
      ...
      return c.name.toLowerCase().includes(q) || c.id.toLowerCase().includes(q) || (c.effect || '').toLowerCase().includes(q);
    }
    return true;
  });
  ...
  return (
    <div className="space-y-8 animate-fade-in py-12">
      ...
      {activeView === 'bydeck' && (
        <ByDeckView onSelectCard={setSelected} />
      )}
...
function ByDeckView({ onSelectCard }: { onSelectCard: (c: Card) => void }) {
  const { PREBUILT_DECKS } = require('../data');
  return (
    <div className="space-y-10">
      {PREBUILT_DECKS.map((deck: any) => {
        ...
        return (
          <div key={deck.id}>
            <div className="mb-4 pb-3 border-b border-[#dfe3e1]/5">
              ...
              <p className="text-sm text-[#c0c8c5]/60 mt-1">{deck.subtitle}</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
...
function VotesView() {
  ...
  const { submitToGoogleForm } = require('../utils/forms');
  const { FORMS, FIELDS } = require('../config');
  const done: any[] = JSON.parse(localStorage.getItem('pb_votes_done') || '[]');
  ...
  function castVote(voteId: string, chosen: string, rejected: string, cardId: string) {
    ...
    const updated = [...done, { id: voteId, cardId, chosen }];
    localStorage.setItem('pb_votes_done', JSON.stringify(updated));
    setVotes(v => [...v]); // force re-render
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      {votes.map((v: any) => {
        const voted = done.find((d: any) => d.id === v.id);
            ...
            {voted ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['A','B'] as const).map(side => {
                  ...
                  const other = v[`option${ side === 'A' ? 'B' : 'A'}`];
                  return (
